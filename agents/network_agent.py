...
class NetworkAgent:
    def __init__(self, alert_manager_instance, session_service: InMemorySessionService):
        ...
        self.adk_agent = Agent(
            ...
            instruction=(
                "You are a Network Security Agent. Your primary role is to monitor "
                "network-related security events (e.g., network connections, data transfers, DNS queries). "
                "For each event you receive, use the `tool_detect_network_anomaly` function to check for anomalies. "
                "If `tool_detect_network_anomaly` returns an anomaly (a JSON string representing it), then immediately "
                ...
                "and specify the source agent (which is 'NetworkAgent'). "
