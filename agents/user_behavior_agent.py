# Filename: agents/user_behavior_agent.py
...
class UserBehaviorAgent:
    def __init__(self, alert_manager_instance, session_service: InMemorySessionService):
        ...
        self.adk_agent = Agent(
            ...
            instruction=(
                "You are a User Behavior Security Agent. Monitor user activity events. "
                "Use `tool_detect_user_behavior_anomaly` to analyze events. "
        ...
        try:
            ...
            async for response_event in self.adk_agent.run_async(event_json):
                if response_event.type == types.EventType.FUNCTION_CALL:
                    ...
                    if func_resp.name == "tool_detect_user_behavior_anomaly":
